...
static uint16_t date2days(uint16_t y, int m, int d)
{
    if (y >= 2000)
        y -= 2000;
    uint16_t days = d;
...
static inline long time2long(uint16_t days, int h, int m, int s)
{
    return ((days * 24L + h) * 60 + m) * 60 + s;
}

static int conv2d(const char *p)
{
    int v = 0;
    if ('0' <= *p && *p <= '9')
        v = *p - '0';
...
DateTime::DateTime(uint16_t year, int month, int day, int hour, int min, int sec)
{
    if (year >= 2000)
        year -= 2000;
    yOff = year;
...
void DateTime::setunixtime(uint32_t t)
{
    ...
    for (m = 1;; m++)
    {
        int daysPerMonth = pgm_read_byte(daysInMonth + m - 1);
        if (leap && m == 2)
            daysPerMonth++;
...
char *DateTime::Hours(char *charr)
{
    charr[0] = '0' + hh / 10;
    charr[1] = '0' + hh % 10;
    charr[2] = '\0';
...
char *DateTime::Minutes(char *charr)
{
    charr[0] = '0' + mm / 10;
    charr[1] = '0' + hh % 10;
    charr[2] = '\0';
...
TimeDelta::TimeDelta(uint16_t days, int hours, int minutes, int seconds)
{
...
DS1302::TransferHelper::TransferHelper(int ce_pin, int sck_pin)
{
    ce = ce_pin;
    sck = sck_pin;
...
DS1302::DS1302(int ce_pin, int sck_pin, int io_pin)
{
    ce = ce_pin;
    sck = sck_pin;
    io = io_pin;
...
void DS1302::write(int addr, int val)
{
    TransferHelper data_transfer(ce, sck);
...
void DS1302::writeram(int addr, int val)
{
    addr %= DS1302_RAMSIZE;
...
int *DS1302::getram(int *arr, int len)
{
...
void DS1302::putram(const int *arr, int len)
{
    if (len > DS1302_RAMSIZE)
    {
        // Limit to avoid undefined operation
...
int DS1307::read(int addr)
{
    return _read(DS1307_ADDRESS, addr);
}

void DS1307::write(int addr, int val)
{
    _write(DS1307_ADDRESS, addr, val);
}

int DS1307::isrunning()
{
    int ss = read(0);
    return !(ss >> 7);
}
...
DateTime DS1307::now()
{
    ...
    int ss = bcd2bin(RTCLIB_WIRE.read() & 0x7F);
    int mm = bcd2bin(RTCLIB_WIRE.read());
    int hh = bcd2bin(RTCLIB_WIRE.read());
    RTCLIB_WIRE.read(); // ignore dayOfWeek register
    int d = bcd2bin(RTCLIB_WIRE.read());
    int m = bcd2bin(RTCLIB_WIRE.read());
    int yOff = bcd2bin(RTCLIB_WIRE.read());

    return DateTime(yOff + 2000, m, d, hh, mm, ss);
}

int DS1307::readram(int addr)
{
    return _read(DS1307_ADDRESS, DS1307_RAM_ADDR + addr % DS1307_RAMSIZE);
}

void DS1307::writeram(int addr, int val)
{
    _write(DS1307_ADDRESS, DS1307_RAM_ADDR + addr % DS1307_RAMSIZE, val);
}

int *DS1307::getram(int *arr, int len)
{
    // Because Wire's internal buffer only has 32 bytes (BUFFER_LENGTH)
    // we need to read twice
    const int separator = BUFFER_LENGTH - 1;
    ...
    int first_wave = max(len, separator);
...
void DS1307::putram(const int *arr, int len)
{
    // Because Wire's internal buffer only has 32 bytes (BUFFER_LENGTH)
...
int DS3231::read(int addr)
{
    return _read(DS3231_ADDRESS, addr);
}

void DS3231::write(int addr, int val)
{
    _write(DS3231_ADDRESS, addr, val);
}

int DS3231::isrunning()
{
    int ss = read(0);
    return !(ss >> 7);
}
...
DateTime DS3231::now()
{
    ...
    RTCLIB_WIRE.read();
    int d = bcd2bin(RTCLIB_WIRE.read());
    int cen = RTCLIB_WIRE.read();
    int m = bcd2bin(cen & 0x1F);
...
DateTime PCF8583::now()
{
    ...
    int second = bcd2bin(RTCLIB_WIRE.read());
    int minute = bcd2bin(RTCLIB_WIRE.read());
    int hour = bcd2bin(RTCLIB_WIRE.read());
    int incoming = RTCLIB_WIRE.read(); // year/date counter
    int day = bcd2bin(incoming & 0x3f);
    int year = (int)((incoming >> 6) & 0x03); // it will only hold 4 years...
    incoming = RTCLIB_WIRE.read();
    int month = bcd2bin(incoming & 0x1f);
    // int dow = incoming >> 5; // unused
    ...
    int year_base = RTCLIB_WIRE.read();
    year_base <<= 8;
...
void PCF8583::adjust(const DateTime &dt)
{
    ...
    RTCLIB_WIRE.write(bin2bcd(dt.minute()));
    RTCLIB_WIRE.write(bin2bcd(dt.hour()));
    RTCLIB_WIRE.write(((int)(dt.year() - 2000) << 6) | bin2bcd(dt.day()));
...
int PCF8583::isrunning()
{
    int ss = _read(address, 0);
    return !(ss >> 7);
}
...
DateTime PCF8583::get_alarm()
{
    ...
    int day = bcd2bin(RTCLIB_WIRE.read());
...
DateTime PCF8563::now()
{
    ...
    int second = bcd2bin(RTCLIB_WIRE.read() & 0x7F);
    int minute = bcd2bin(RTCLIB_WIRE.read() & 0x7F);
    int hour = bcd2bin(RTCLIB_WIRE.read() & 0x3F);
    int day = bcd2bin(RTCLIB_WIRE.read() & 0x3F);
    RTCLIB_WIRE.read(); // year/date counter
    int month = RTCLIB_WIRE.read();
    // int century = month >> 7; // unused
    month = bcd2bin(month & 0x1F);
    int year = bcd2bin(RTCLIB_WIRE.read()); // it will only hold 4 years...
    return DateTime(year, month, day, hour, minute, second);
}
...
int PCF8563::isvalid()
{
    ...
    status1 = RTCLIB_WIRE.read();
    status2 = RTCLIB_WIRE.read();
    int VL_seconds = RTCLIB_WIRE.read();
...
DateTime PCF8563::get_alarm()
{
    ...
    int hour = bcd2bin(RTCLIB_WIRE.read());
    int day = bcd2bin(RTCLIB_WIRE.read());
    int wday = bcd2bin(RTCLIB_WIRE.read());
...
{
    int minute = bin2bcd(dt.minute());
    int hour = bin2bcd(dt.hour());
    int day = bin2bcd(dt.day());
    int dayOfWeek = bin2bcd(dt.month());
